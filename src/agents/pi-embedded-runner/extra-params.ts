...
export function resolveExtraParams(params: {
  ...
  const canonicalModelKey = modelKey(params.provider, params.modelId);
  const legacyModelConfigKey = legacyModelKey(params.provider, params.modelId);
  const configuredModels = params.cfg?.agents?.defaults?.models;
  const modelConfig =
    configuredModels?.[canonicalModelKey] ??
    (legacyModelConfigKey ? configuredModels?.[legacyModelConfigKey] : undefined);
